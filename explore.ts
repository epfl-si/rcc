...
const validateURL = (link: string): vURL => {
  ...
  let urlInfo = {'tld': '', 'domain': '', 'sub': ''}
  try {
    urlInfo = tld(url.toString(), {'allowUnknownTLD' : true})
  } catch (e) {
    console.error(e)
  }
...
const discover = async (filePath: string, opts: any): Promise<report> => {
  ...
  const browser = await puppeteer.launch({
    headless: true,
    args: ['--no-sandbox', '--start-maximized', `--window-size=${opts.dimension}`], // defaut size is 800x600 and the "mobile" menu can be displayed
